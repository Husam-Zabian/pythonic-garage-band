...
#@pytest.mark("todo")
def test_bandist_str():
    metallica = Band("Metallica",[])
    actual = str(metallica)
    expected = "our team is Metallica and we play Rock music"
    assert actual == expected


#@pytest.mark.skip("todo")
def test_bandist_repr():
    metallica = Band("Metallica",[])
    actual = repr(metallica)
    expected = "The Memory Remains. BandName = Metallica"
    assert actual == expected
